...
module.exports.userinfoadd = function (req, res, next) {
    if (req.body && typeof req.body == "object") {
        for (const key of requiredBodyFields) {
            if (!(key in req.body)) res.status(406).send('Field ' + key + ' is empty!');
        }
        userinfoadd(req)
            .then(data => res.status(201).send('User ' + data.firstname + ' ' + data.lastname + ' was created.'))
            .catch(err => res.status(406).send(err))
    } else  res.status(406).send('Empty body');
};
